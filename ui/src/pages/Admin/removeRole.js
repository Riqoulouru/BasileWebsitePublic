...
export default function RemoveRolePage() {
    ...
    useEffect(() => {
        ...
        const fetchUserRole = async () => {
            ...
            fetch('https://basilethiry.fr/back/user/getRole', options)
                .then(response => response.json())
                .then(data => {
                    console.log(data);
                    console.log(typeof data);
                    setRolesNames(data);
                    setSelectedRole(data.length > 0 ? data[0].name : ''); // Update selected role
    ...
    const handleRemoveRole = async () => {
        ...
        try {
            const response = await fetch('https://basilethiry.fr/back/user/removeRole', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
            ...
            if (response.ok) {
                setError('');
                alert("Le rôle a bien été supprimé à l'utilisateur");

                setRolesNames(roleNames.filter(role => role.name !== selectedRole));
                setSelectedRole(roleNames[0].name);
    ...
    useEffect(() => {
        const fetchLoginStatus = async () => {
            ...
            if (roles != null) {
                setUserRoles(JSON.parse(roles)); // Parse the string to convert it into an array
            }
        };
            ...
            {loadAuthorized ? (
                <div className={styles.container}>
                    <div className={styles.square}>
                        <h2 className={styles.title}>Supprimer un rôle</h2>
                        ...
                        <button className={styles.button} onClick={handleRemoveRole}>Valider</button>
